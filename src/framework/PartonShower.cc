...
vector<fjcore::PseudoJet> PartonShower::GetFinalPartonsForFastJet()
{
  ...
  for (int i = 0; i < mP.size(); i++)
  {
    ...
    if (mP[i]->has_user_info<HQInfoBase>())
    {
      forFJ[i].set_user_info(new HQInfoBase(mP[i]->user_info<HQInfoBase>().hq_channel(), mP[i]->user_info<HQInfoBase>().hq_mother_id()));
    }
  }
...
{
  *o << "label "
     << "\"" << n.id() << "(" << fixed << setprecision(2) << vMap[n]->x_in().t() << ")\"" << endl;
...
void PartonShower::SaveAsGV(string fName)
{
  ...
  for (eIt = edges_begin(), eEnd = edges_end(); eIt != eEnd; ++eIt)
  {
    ...
    gv << (to_string(eIt->source().id()) + "->" + to_string(eIt->target().id())) << " " << label
